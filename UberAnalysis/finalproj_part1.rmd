...
In the dashboard, I will be covering various US cities. But for the purpose of this submission, I will be doing a time series analysis on San Francisco's Daily uber trips data from 2nd January 2016 to 31 March 2020. This was obtained from Uber Movement data.

```{r}
sanfranscico <- fread("C://Users//sabad//OneDrive//Documents//Time Series Analysis//Project//Data//formatted_sanfrancisco.csv")
```
...
```{r}
avg_times_ts %>% gg_season(mean_travel_time) + labs(y="Mean Travel Time",x="Month",title="Seasonal plot of mean travel times")
```
...
```{r}
avg_times_ts %>% gg_season(mean_travel_time,period="week") + labs(y="Mean Travel Time",x="Day",title="Seasonal plot showing daily seasonal patterns of mean travel times")
```
...
```{r}
trip_count_ts %>% gg_season(count_trips) + labs(y="Number of Trips",x="Month",title="Seasonal plot of number of destination zones")
```
...
```{r}
trip_count_ts %>% gg_season(count_trips,period="week") + labs(y="Number of Trips",x="Day",title="Seasonal plot of number of destination zones")
```
...
We are seeing correlations in lag 7, lag 14, etc. that is on a weekly basis. So we will be applying one more order of differencing.

## Doubly differenced

```{r}
avg_times_ts %>% gg_tsdisplay(difference(mean_travel_time,31) %>% difference() ,plot_type = "partial",lag=36)
```
...
Here we are seeing some minor spikes. The significant spike at in lag 31 in the ACF suggests a seasonal MA(1) component

```{r}
fit <- avg_times_week %>%
  model(arima011011 = ARIMA(mean_travel_time ~ pdq(0,1,1) + PDQ(0,1,1)),
    arima310010 = ARIMA(mean_travel_time ~ pdq(3,1,0) + PDQ(0,1,0)),
...
augment(fit) %>%
  ...
  features(.innov, ljung_box, lag=52, dof=4)
```

Here we see there are some leftover residuals. 
...
```{r}
fit_hr %>% forecast(h="2 years") %>% autoplot(avg_times_ts,level=95) + 
